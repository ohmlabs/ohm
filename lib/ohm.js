(function() {
  ...
  function Ohm(config) {
    var ParseDashboard, ParseServer, SessionDataStore, io, ohmSessionDataStore, ensureAuthenticatedSocket, ohmCookieParser, S3Adapter, session, server, helmet, bodyParser, cookieParser, errorHandler, express, ghost, http, logger, methodOverride, parentApp, path;
    ...
    if (config.is_prod) {
      console.log('Server starting on port ' + config.port + ' in ' + parentApp.settings.env + ' mode');
    } else {
      console.log('Server starting on port ' + config.port + ' in ' + parentApp.settings.env + ' mode');
    }
    ///////////////////////////////////
